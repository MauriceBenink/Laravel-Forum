...
@section('content')
    <form action="{{url('specialPermission/add')}}" method="POST">
        {{ csrf_field() }}
        <input type="hidden" name ="info" value="{{$group[0]->user_group_id}}">
    ...
    <br><br>
        <ul id="list-container" style="float:left">
            ...
            <li class = "main-list" style = "float:left;margin-left:10px;list-style-type:none">
                <ul class="user-group">
                    <li style="list-style-type: none">Content Groups</li>
                    @if(!is_null($content))
                        @foreach($content as $solo)
                            <li style="list-style-type: none">
                                <input type="checkbox" name="add[content][]" value="{{$solo->content_group_id}}">{{$solo->group_name}}
                            </li>
                        @endforeach
                    @endif
                </ul>
